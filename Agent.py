...
class Agent:
    def __init__(self, spatial_size, hero_size, monster_size, turret_size, minion_size, stat_size, invisible_size):
        self.lstm_time_steps = Config.lstm_step
        self.lstm_unit_size = AgentConfig.lstm_unit_size
        ...
        self.invisible_dim = AgentConfig.invisible_dim
        self.action_query_dim = AgentConfig.action_query_dim
        self.action_key_dim = AgentConfig.action_key_dim
        self.human_embedding_size = AgentConfig.human_embedding_size

        self.action_size_list = AgentConfig.action_size_list
        self.value_head = AgentConfig.value_head_list
    ...
    def build(self, is_train):
        with tf.variable_scope('Agent'):
            ...
            concat_all_size = spatial_flatten_size + self.action_query_dim * 4 + self.stat_dim // 4
            ...
            lstm_human_size = self.lstm_unit_size + self.human_embedding_size
            self.fc_lstm_human_weight = _fc_weight_variable(shape=[lstm_human_size, lstm_human_size // 4], name="fc_lstm_human_weight", trainable=is_train)
            self.fc_lstm_human_bias = _bias_variable(shape=[lstm_human_size // 4], name="fc_lstm_human_bias", trainable=is_train)
            ...
            for label_index in range(len(self.action_size_list)):
                if (label_index == (len(self.action_size_list) - 1)):
                    fc_label_weight = _fc_weight_variable(shape=[lstm_human_size // 4, self.action_key_dim], name="fc_label_%d_weight" % (label_index))
                    fc_label_bias = _bias_variable(shape=[self.action_key_dim], name="fc_label_%d_bias" % (label_index))
                else:
                    fc_label_weight = _fc_weight_variable(shape=[lstm_human_size // 4, self.action_size_list[label_index]], name="fc_label_%d_weight" % (label_index))
            ...
            # value
            concat_state_size = lstm_human_size // 4 + self.invisible_dim // 4
            self.fc_value_weight = _fc_weight_variable(shape=[concat_state_size, self.value_head], name="fc_value_weight", trainable=is_train)
            self.fc_value_bias = _bias_variable(shape=[self.value_head], name="fc_value_bias", trainable=is_train)

    def infer(self, agent_feature_list, human_embedding, task_gate):
        with tf.variable_scope('Agent'):
            public_embed_list = []
            private_embed_list = []
            ...
            for player_index in range(self.agent_num):
                ...
                concat_all = tf.concat([flatten_conv2_result, pool_hero_result, pool_monster_result, pool_turret_result, pool_minion_result, fc2_stat_result], axis=1, name="concat_all")
            ...
            for player_index in range(self.agent_num):
                ...
                # concat lstm and human embedding
                concat_lstm_human = tf.concat([reshape_lstm_outputs_result, human_embedding * task_gate], axis=1, name="concat_lstm_human")
                fc_lstm_human_result = tf.nn.relu((tf.matmul(concat_lstm_human, self.fc_lstm_human_weight) + self.fc_lstm_human_bias), name="fc_lstm_human_result_%d" % player_index)
                lstm_human_list.append(fc_lstm_human_result)
                ...
                with tf.variable_scope("player%d_action" % player_index):
                    ...
                    for action_index in range(len(self.action_size_list)):
                        ...
                        if (action_index == (len(self.action_size_list) - 1)):
                            fc_action_key = tf.add(tf.matmul(lstm_human_list[player_index], self.fc_action_weight_list[action_index]), self.fc_action_bias_list[action_index], name="player%d_fc_action_key" % (player_index))
                            ...
                            action_logits = tf.add(tf.matmul(lstm_human_list[player_index], self.fc_action_weight_list[action_index]), self.fc_action_bias_list[action_index], name="player%d_fc_action_%d_result" % (player_index, action_index))
                        softmax_prob = tf.nn.softmax(action_logits)
                        one_action_list.append(softmax_prob)
                    agent_action_list.append(one_action_list)

                with tf.variable_scope("player%d_value" % player_index):
                    # value output
                    concat_state = tf.concat([lstm_human_list[player_index], invisible_embed_list[player_index]], axis=1, name="concat_state")
                    ...
                    agent_value_list.append(fc_value_result)

        return agent_action_list, agent_value_list